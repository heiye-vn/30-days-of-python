...
**Q**：上述示例为什么选择多进程（Process）而非多线程（Thread）？

**A**：在 Python 中，受 **GIL（Global Interpreter Lock，全局解释器锁）** 的限制，同一时刻只有一个线程能在 CPU 上运行 Python 字节码。

- 如果使用多线程执行此类 CPU 密集型任务，多个线程会为了争夺 GIL 锁而频繁切换，不仅无法利用多核 CPU，反而会因为线程切换的开销导致运行速度变慢
- 使用 `ProcessPoolExecutor` 可以创建多个独立的 Python 进程，每个子进程都拥有独立的 GIL 锁和内存空间，从而能够实现真正的**多核并行计算**，显著缩短总计算时间
